...
@section('title', 'Types de Contenu - Culture Bénin')
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12" data-aos="fade-down">
            ...
            <p class="text-xl text-gray-600 max-w-2xl mx-auto">
                Gérez les différentes catégories de contenus culturels
            </p>
        </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                <div class="flex items-center space-x-6">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-benin-green">{{ $typesContenu->count() }}</div>
                        <div class="text-sm text-gray-500">Types de contenu</div>
                    </div>
                    <div class="hidden md:block w-px h-8 bg-gray-300"></div>
                    <div class="text-center">
                        @php
                            $totalContenus = $typesContenu->sum('contenus_count');
                        @endphp
                        <div class="text-2xl font-bold text-benin-yellow">{{ $totalContenus }}</div>
                        <div class="text-sm text-gray-500">Contenus associés</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-8" data-aos="fade-up" data-aos-delay="100">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Ajouter un type de contenu</h3>
            <form action="{{ route('types-contenu.store') }}" method="POST" class="flex space-x-4">
                @csrf
                <div class="flex-1">
                    <input type="text" 
                           name="nom" 
                           ...
                           placeholder="Ex: Histoire, Recette, Tradition..."
                           value="{{ old('nom') }}">
                    @error('nom')
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <form action="{{ route('types-contenu.destroy', $type) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                    ...
                                    @else
                                    <span class="text-gray-400 cursor-not-allowed" title="Impossible de supprimer - des contenus sont associés">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center">
                                ...
                                <h3 class="text-2xl font-bold text-gray-900 mb-4">Aucun type de contenu</h3>
                                <p class="text-gray-600 mb-8 max-w-md mx-auto">
                                    Commencez par créer le premier type de contenu.
                                </p>
                            </td>
                        </tr>
                        @endforelse
