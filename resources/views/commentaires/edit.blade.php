...
@section('title', 'Modifier le Commentaire - Culture Bénin')
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12" data-aos="fade-down">
            <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
                Modifier le <span class="text-benin-green">Commentaire</span>
            </h1>
            <p class="text-xl text-gray-600">
                Mettez à jour votre contribution
            </p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden" data-aos="fade-up">
            <div class="benin-gradient px-6 py-4">
                <h2 class="text-xl font-bold text-white">Modification du commentaire</h2>
            </div>

            <form action="{{ route('commentaires.update', $commentaire) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div data-aos="fade-up" data-aos-delay="100">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <h3 class="font-semibold text-gray-900">{{ $commentaire->contenu->titre }}</h3>
                        <p class="text-sm text-gray-600 mt-1">
                            {{ Str::limit($commentaire->contenu->texte, 100) }}
                        </p>
                    </div>
                </div>
                ...
                <div data-aos="fade-up" data-aos-delay="150">
                    ...
                    <textarea id="texte" 
                              ...
                              placeholder="Partagez vos pensées, vos questions ou vos compléments d'information...">{{ old('texte', $commentaire->texte) }}</textarea>
                    @error('texte')
                ...
                <div data-aos="fade-up" data-aos-delay="200">
                    ...
                    <div class="flex items-center space-x-1" id="note-container">
                        @for($i = 1; $i <= 5; $i++)
                            <button type="button" 
                                    data-note="{{ $i }}"
                                    class="note-star w-8 h-8 {{ $i <= old('note', $commentaire->note) ? 'text-benin-yellow' : 'text-gray-300' }} transition-colors duration-200">
                    ...
                    <input type="hidden" name="note" id="note" value="{{ old('note', $commentaire->note) }}">
                    <p class="mt-1 text-sm text-gray-500">Cliquez sur les étoiles pour modifier la note</p>
                </div>
            ...
            <!-- Delete Form -->
            <form id="delete-form" action="{{ route('commentaires.destroy', $commentaire) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
